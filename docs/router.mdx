...
2. Next.js `i18n` is full of bugs, and in the latest versions, they completely abandoned it, recommending using proxy.

3. In `proxy`, you can decide for yourself which settings to enable for which paths, which rules to set, which redirect lists for `/app` paths, and which for `/marketing`.
   Take a look at the [proxy-chain](./proxy-chain.mdx) to get the most flexible and suitable configuration for you.
...
> [!TIP]
> For flexibility, it is recommended not to use basePath in next.config.js, but
> to manually set the prefix in the links.

`i18n` - configuration for localized paths.

> [!TIP]
> Unlike the next.config.js solution, it has nothing to do with Link and does
> not participate in the generation of page paths. That is, localization should
> be embedded in the structure of the application (e.g. `app/[lang]/page.tsx`).
...
> [!TIP]
> It is recommended not to use i18n in next.config.js, but to manually set the
> required locale in the links.
