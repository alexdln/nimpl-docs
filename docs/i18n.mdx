...
<Hint>
  To use i18n on the client side, you need to set up the
  [I18nTransmitter](#i18ntransmitter)
</Hint>
...
<Hint>
  The client hook and component, unlike the server ones, do not support language
  customization. This is because translations are determined by server parents,
  as a result, the language is set in them.
</Hint>
...
<Hint>
  It is recommended to set it at the top level of the route. However, in some
  cases (*for example in conditions for different sections*) it may be useful to
  add a transmitter in another place.
</Hint>
...
const ServerComponent: React.FC = () => (
  <I18nTransmitter
    terms={[
      [
        "home.welcome",
        { stage: process.env.GITHUB_REF === "main" ? "production" : "test" },
      ],
    ]}
  >
    <ClientComponent />
...
```tsx
<ServerTranslation term="intro.description" language="fr" />
```
...
<Hint>
  The ClientTranslation and client hook [useTranslation](#usetranslation),
  unlike server [getTranslation](#gettranslation) and
  [ServerTranslation](#servertranslation), do not support language
  customization. This is because translations are determined by server parents,
  as a result, and the language is fixed in them.
</Hint>
...
However, if there is no way to transfer the file or the hosting has internal blocks (_for example, Vercel considers this file unused and does not read it_) - you need to wrap `next.config.js` in `withI18n`
