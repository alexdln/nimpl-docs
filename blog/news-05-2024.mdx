...
A new category was added, tools - utilities that can make life better. At the moment, they include [@nimpl/versioner](https://github.com/alexdln/nimpl-versioner) and [@nimpl/inio](https://github.com/alexdln/nimpl-inio).

## New package - @nimpl/proxy-chain

During the work on the package [@nimpl/ab-tests](https://github.com/alexdln/nimpl-ab-tests), it was decided to test it in several projects. However, adding a simple middleware turned out to be a real adventure. The problem is that there were already middleware in the projects - one with next-intl, one with next-auth.
...
But it was necessary for each middleware to work like the original middleware from next.js. That's how [@nimpl/proxy-chain](https://github.com/alexdln/nimpl-proxy-chain) appeared:
...
That's why I appreciated the idea of creating a dynamic new way to edit JSON files. A Plug-and-Play solution - just Run and Edit! With [@nimpl/inio](https://github.com/alexdln/nimpl-inio).
